...
def one_episode(episode_no, env, agent, episodes):
    ...
    for itr in range(env.num_layers + 1):
        ...
        if done:
            if episode_no % 20 == 0:
                print(
                    "episode: {}/{}, score: {}, e: {:.2}, rwd: {} \n".format(
                        episode_no, episodes, itr, agent.epsilon, reward
                    )
                )
            break
...
def train(agent, env, episodes, seed, output_path, threshold):
    ...
    for e in range(episodes):
        ...
        if e % 50 == 0 and e > 0:
            ...
            torch.save(
                agent.policy_net.state_dict(),
                f"{output_path}/thresh_{threshold}_{seed}_model.pth",
            )
            torch.save(
                agent.optim.state_dict(),
                f"{output_path}/thresh_{threshold}_{seed}_optim.pth",
            )
...
def get_args(argv):
    ...
    parser.add_argument(
        "--config", type=str, default="h_s_2", help="Name of configuration file"
    )
    parser.add_argument(
        "--experiment_name",
        type=str,
        default="lower_bound_energy/",
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Set specific GPU to run experiment [0, 1, ...]",
    )
...
if __name__ == "__main__":
    ...
    pathlib.Path(f"{results_path}{args.experiment_name}{args.config}").mkdir(
    ...
    loss_dict, scores_dict, test_scores_dict, actions_dict = (
        dict(),
        dict(),
        dict(),
        dict(),
    ...
    agent = agents.__dict__[conf["agent"]["agent_type"]].__dict__[
        conf["agent"]["agent_class"]
    ](conf, environment.action_size, environment.state_size, device)
    ...
    train(
        agent,
        environment,
        conf["general"]["episodes"],
        args.seed,
        f"{results_path}{args.experiment_name}{args.config}",
        conf["env"]["accept_err"],
    )
    agent.saver.save_file()
