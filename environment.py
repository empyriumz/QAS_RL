...
class CircuitEnv:
    def __init__(self, conf, device):
        ...
        self.fake_min_energy = (
            conf["env"]["fake_min_energy"]
            if "fake_min_energy" in conf["env"].keys()
            else None
        )
        ...
        __ham = np.load(
            f"mol_data/LiH_{self.num_qubits}q_geom_{self.geometry}_{self.ham_mapping}.npz"
        )
        hamiltonian, eigvals, energy_shift = (
            __ham["hamiltonian"],
            __ham["eigvals"],
            __ham["energy_shift"],
        )
    ...
    def step(self, action, train_flag=True):
        ...
        if next_state[2][self.actual_layer] != self.num_qubits:
            ...
            if self.optim_method == "Rotosolve_local_end_only_rot":
                thetas_to_optim = min(
                    next_state[-1][next_state[2] != self.num_qubits].size()[0],
                    self.local_opt_size,
                )
        ...
        if self.optim_method == "Rotosolve_local_end":
            ...
            nb_of_thetas = next_state[-1][next_state[2] != self.num_qubits].size()[
                0
            ]  ## number of all thetas
            thetas_to_optim = min(
                nb_of_thetas, self.local_opt_size
            )  ## number of thetas which we want to optimize
            angle_indices = np.arange(
                nb_of_thetas - thetas_to_optim, nb_of_thetas
            )  ## in COBYLA case we need them in ascending order and positive (not [-1,-2,-,3])
            # print(angle_indices)
    ...
    def reset(self):
        ...
        self.hamiltonian, eigvals, self.energy_shift = (
            __ham["hamiltonian"],
            __ham["eigvals"],
            __ham["energy_shift"],
        )
        ...
        self.min_eig = (
            ...
            else min(eigvals) + self.energy_shift
        )
    ...
    def make_circuit(self, thetas=None):
        ...
        assert (
            circuit.get_gate_count() <= self.num_layers
        ), "Wrong circuit construction, too many gates!!!"
        return circuit
    ...
    def global_roto(self, which_angles=[]):
        ...
        qulacs_inst = VQE.Parametric_Circuit(n_qubits=self.num_qubits)
        ...
        for _ in range(self.global_iters):
            for i in which_angles:
                ...
                e1 = VQE.get_energy_qulacs(theta1, *arguments)
                e2 = VQE.get_energy_qulacs(theta2, *arguments)
                e3 = VQE.get_energy_qulacs(theta3, *arguments)
                ## Energy lanscape is of the form A \sin( \theta + B) + C
                C = 0.5 * (e1 + e3)
    ...
    def scipy_optim(self, method, which_angles=[]):
        ...
        if list(which_angles):
            ...
            state[-1][state[2] != self.num_qubits] = torch.tensor(
                result_min_qulacs["x"], dtype=torch.float
            )
    ...
    def reward_fn(self, energy):
        if self.fn_type == "staircase":
            return (
                0.2 * (self.error < 15 * self.done_threshold)
                + 0.4 * (self.error < 10 * self.done_threshold)
                + 0.6 * (self.error < 5 * self.done_threshold)
                + 1.0 * (self.error < self.done_threshold)
            ) / 2.2
        elif self.fn_type == "two_step":
            return (
                0.001 * (self.error < 5 * self.done_threshold)
                + 1.0 * (self.error < self.done_threshold)
            ) / 1.001
            ...
            return 0.0 + 1.0 * (self.error < self.done_threshold)
