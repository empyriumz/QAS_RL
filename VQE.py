    ...
    def construct_ansatz(self, state):
        ...
        for i in range(state.shape[1]):
            if state[0][i].item() != self.n_qubits:
                self.ansatz.add_CNOT_gate(
                    int(state[0][i].item()), int(state[1][i].item())
                )
            elif state[2][i].item() != self.n_qubits:
                axis = int(state[3][i].item())
                if axis == "X" or axis == "x" or axis == 1:
                    self.ansatz.add_parametric_RX_gate(
                        int(state[2][i].item()), thetas[i]
                    )
                elif axis == "Y" or axis == "y" or axis == 2:
                    self.ansatz.add_parametric_RY_gate(
                        int(state[2][i].item()), thetas[i]
                    )
                elif axis == "Z" or axis == "z" or axis == 3:
                    self.ansatz.add_parametric_RZ_gate(
                        int(state[2][i].item()), thetas[i]
                    )
        assert (
            self.ansatz.get_gate_count() <= state.shape[1]
        ), "Wrong circuit construction, too many gates!!!"
        return self.ansatz


def get_energy_qulacs(
    angles, observable, circuit, n_qubits, energy_shift, which_angles=[]
):
    """"
